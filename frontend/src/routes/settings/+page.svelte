...
<div class="page">
  ...
  <div class="settings-layout">
    <aside class="system-card panel"><div class="panel-head"><div><strong>Система</strong><span>DNS Monitor</span></div><span class="state-chip {$snapshot.capture_error?'error':'good'}">{$snapshot.capture_error?'ERROR':'OK'}</span></div><div class="system-row"><span>DNS Monitor</span><strong>v{$snapshot.version||'—'}</strong></div><div class="system-row"><span>Статус</span><strong>{primaryDown?'ERROR':primaryDegraded?'DEGRADED':'OK'}</strong></div><div class="system-row"><span>Основные DNS</span><strong>{fmtInt(primaryUpstreams)}</strong></div><div class="system-row"><span>Uptime</span><strong>{fmtDuration($snapshot.uptime_seconds)}</strong></div><div class="system-row"><span>Запросы</span><strong>{fmtInt($snapshot.total_requests)}</strong></div><div class="system-row"><span>Discovery</span><strong>{fmtAgo($snapshot.last_discovery)}</strong></div><div class="system-row"><span>RAM</span><strong>{system?.rss_kb?`${(system.rss_kb/1024).toFixed(1)} MB`:'—'}</strong></div></aside>

    <div class="settings-stack">
      <section class="settings-section panel"><div class="settings-title">Общие</div><div class="setting-row"><div><strong>Уровень интерфейса</strong><span>Обычный показывает только основные DNS и рабочие показатели. Расширенный добавляет policy-context, mark/table, внутренние порты и диагностические поля.</span></div><select value={$settings.uiLevel} onchange={(e)=>update('uiLevel',e.currentTarget.value)}><option value="normal">Обычный</option><option value="advanced">Расширенный</option></select></div><div class="setting-row"><div><strong>Масштаб интерфейса</strong><span>Auto автоматически делает UI крупнее на 2K/4K без browser zoom.</span></div><select value={$settings.uiScale} onchange={(e)=>update('uiScale',e.currentTarget.value)}><option value="auto">Auto · рекомендуется</option><option value="100">100%</option><option value="115">115%</option><option value="125">125%</option><option value="140">140%</option></select></div><div class="setting-row"><div><strong>Live update</strong><span>Частота SSE snapshot. При проблемах SSE автоматически включается polling.</span></div><select value={$settings.refreshMs} onchange={(e)=>update('refreshMs',Number(e.currentTarget.value))}><option value="1000">1 сек</option><option value="2000">2 сек</option><option value="5000">5 сек</option><option value="10000">10 сек</option></select></div></section>
      ...
      <section class="settings-section panel"><div class="settings-title">Мониторинг</div><div class="setting-row static"><div><strong>Discovery</strong><span>Основной DNS, System DoT/DoH и служебные policy-context Keenetic.</span></div><code>60 сек</code></div><div class="setting-row static"><div><strong>Health-check</strong><span>Основные resolver'ы и policy-context только при наличии маршрута наружу.</span></div><code>30 сек</code></div><div class="setting-row static"><div><strong>История</strong><span>Minute buckets хранятся только в RAM.</span></div><code>24 часа</code></div><div class="setting-row static"><div><strong>Веб-порт</strong><span>Панель DNS Monitor.</span></div><code>2233</code></div></section>
    </div>
  </div>
</div>
