<script>
  ...
  const moduleDefs = [
    { id: 'system', name: 'System Monitor', short: 'SYS', package: 'dns-monitor-system' },
    { id: 'thermal', name: 'Thermal Monitor', short: 'TMP', package: 'dns-monitor-thermal' },
    { id: 'storage', name: 'Storage Monitor', short: 'DSK', package: 'dns-monitor-storage' },
    { id: 'network', name: 'Network Monitor', short: 'NET', package: 'dns-monitor-network' },
    { id: 'profiling', name: 'Profiling', short: 'PRF', package: 'dns-monitor-profiling' }
  ];
  ...
  function selectTab(id) {
    if (tab === id) return;
    tab = id;
    replaceState(`/modules?tab=${encodeURIComponent(id)}`, {});
    startTimer();
    loadCurrent();
  }
