...
func main() {
	...
	go func() {
		// Discovery happens immediately; give it a moment before capture starts.
	...
	go func() {
		// Client attribution is an independent passive capture path. Failure here
		// must never disable the core loopback DNS monitor.
